...
interface RoofCanvasProps {
  ...
  addAreaTrigger?: number; // 新しい面を追加する合図
}
...
export default function RoofCanvas({ onPointsConfirmed, savedFaces, simulateTrigger, calibLength = 10, onTogglePanel, addAreaTrigger }: RoofCanvasProps) {
  ...
  const [areaPoints, setAreaPoints] = useState<number[]>([]); // 現在描いているエリア
  ...
  // 親画面から「面を追加」ボタンが押されたら、モードをエリア作成に戻す
  useEffect(() => {
  ...
  const handleNextStep = () => {
    if (step === 'reference' && refPoints.length === 4) {
      ...
      setAreaPoints([]); // 描画完了したら現在エリアをクリア（親のsavedFacesに移るため）
    }
  };
      ...
      <div className="flex justify-between items-center mb-2 px-4 flex-none">
        ...
        <div className="space-x-2 flex">
          ...
          {step === 'area' && areaPoints.length >= 6 && (
            <button onClick={handleNextStep} className="px-4 py-2 bg-blue-600 text-white font-bold rounded-md text-sm shadow-sm">この面を確定する</button>
          )}
          {step !== 'done' && (
      ...
      >
        ...
        >
          <Layer>
            ...
            {/* 確定済みのすべての面（青枠とパネル）を描画 */}
            {savedFaces.map((face, fIndex) => (
              <React.Fragment key={`face-${fIndex}`}>
                {face.areaPoints.length > 0 && (
                  <Line points={face.areaPoints} fill="rgba(59, 130, 246, 0.3)" stroke="#3b82f6" strokeWidth={6 / stageScale} closed={true} />
                )}
                {face.placedPanelsCoords.map((pts, pIndex) => {
                  const isActive = face.activePanels[pIndex];
                  return (
                    <Line 
                      key={`panel-${fIndex}-${pIndex}`} 
            ...
            {/* 描画中エリアの長さテキスト */}
            {areaPoints.length >= 4 && pxToMeter > 0 && step === 'area' && (() => {
              const labels = [];
              for (let i = 0; i < areaPoints.length; i += 2) {
                if (i === areaPoints.length - 2) continue;
                const nextI = i + 2;
                const lenPx = Math.hypot(areaPoints[nextI] - areaPoints[i], areaPoints[nextI+1] - areaPoints[i+1]);
                labels.push(
                  <Text
                    key={`len-${i}`} x={(areaPoints[i] + areaPoints[nextI]) / 2} y={(areaPoints[i+1] + areaPoints[nextI+1]) / 2}
                    text={`${(lenPx * pxToMeter).toFixed(1)}m`}
                    fontSize={15 / stageScale} fill="#1d4ed8" stroke="#ffffff" strokeWidth={4 / stageScale} fillAfterStrokeEnabled fontStyle="bold" offsetX={20 / stageScale} offsetY={15 / stageScale}
                  />
                );
              }
              return labels;
              ...
              if (index % 2 === 0) {
                return (
                  <Circle 
                    ...
                    radius={7.5 / stageScale} fill="#ef4444" stroke="#ffffff" strokeWidth={2 / stageScale}
                    draggable listening
                    onDragMove={(e) => {
